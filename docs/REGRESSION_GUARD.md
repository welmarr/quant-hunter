...
|---|---|---|---|---|---|
| REG-F01 | `IMPLEMENTED / REVIEW PENDING — SOFTWARE ONLY` | A released sealed partition becomes one-way `EXPOSED` and can never be resealed. | DEC-0006; DEC-0031; Roadmap Item 10A | `isolation/ledger.py`, `isolation/release.py`; release, repeat-release, accidental/partial-exposure, chain, CAS, and immutable-status tests in `test_sealed_release.py` | Invalidate affected experiments and stop release. |
| REG-F02 | `IMPLEMENTED / REVIEW PENDING — SOFTWARE ONLY` | Viewing sealed results terminates search/tuning for that experiment; changes require a new experiment and genuinely untouched evidence where possible. | `EXPERIMENT_LEDGER.md`; `VALIDATION_STANDARD.md`; DEC-0031 | `experiments/lifecycle.py`; release-reference retention, post-release attempt rejection, single-FROZEN, and zero-attempt fixed-evaluation tests in `test_sealed_release.py` | Later result is exploratory or invalid, never confirmatory. |
