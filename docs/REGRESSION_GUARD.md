...
| ID | Status | Invariant | Governing authority | Required future evidence | Failure consequence |
|---|---|---|---|---|---|
| REG-F03 | `TOOLING IMPLEMENTED / HOST EVIDENCE BLOCKED` | A `HOST_ENFORCED` release requires typed canonical evidence created by one governed executed Windows preflight/setup/verification flow, exact Item 8 FROZEN authority, the verified release root and artifact, and the same global one-way exposure ledger. Raw mappings, JSON documents, and arbitrary report files cannot create authority. The real Windows research identity must be unable to list, read, create, write, delete, own, or change ACLs on the sealed vault before release. | DEC-0006; DEC-0035–DEC-0036; RISK-017; Roadmap Item 10B | Hostile schema/service/raw-promotion tests and inert-by-default scripts exist. Still required: independent review of the corrective authority path, then elevated proof of an already encrypted fixed NTFS volume, exact preflight binding, effective two-identity DACL/SACL, audit events, backup/sync/index evidence, denial, and controlled release. | Item 10 and Stage 1B cannot pass. |
