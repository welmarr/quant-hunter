...
**Status: IN PROGRESS — ITEM 8A REVIEW FIX (2026-09-06).** Batches 1–2
...
`952bd3a4a30518d51b6a9dbe679b00f9c28753fd`. Item 8 is `IN PROGRESS` only for
the explicitly authorized 8A lifecycle and freeze core; items 9–13 remain
unauthorized and unstarted. Execute
...
8. **Experiment controls — IN PROGRESS / ITEM 8A REVIEW FIX:** Item 8A implements only governed `DRAFT → REGISTERED → FROZEN` transitions, a result-free preregistration gate, exact registered-revision freeze binding, caller-supplied full-precision UTC transition timestamps, and immutable canonical freeze evidence. Item 8A remains pending independent review. Multiple-testing runtime attempt counters, execution and later transitions, release-event integration, result/failure retention, and deterministic `rerun EXP-<uuidv7>` resolution remain separately gated. Do not implement a backtesting engine or strategy.
