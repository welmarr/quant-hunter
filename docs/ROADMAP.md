...
**Status: IN PROGRESS — ITEM 9B IN PROGRESS / REVIEW FIX (2026-09-07).** Batches 1–2
...
`5636ad431b1c660233189a45ebfd6157308df7fa`. Item 9 is now in progress through
Item 9B scientific-evidence review fix; Item 9C and items 10–13 remain unstarted. Execute
...
9. **Validation and simulation interfaces only — IN PROGRESS / ITEM 9B REVIEW FIX:** Item 9A is `COMPLETE / INDEPENDENT REVIEW PASSED` and defines immutable, configuration-driven metadata contracts for exact UTC half-open intervals, chronological partitions, explicitly sequenced time-aware folds, and fold-local purge/embargo evidence. Final hosted evidence was 488 passing tests on Ubuntu and Windows, 91.46% Ubuntu combined statement/branch coverage, and 99.34% coverage for `temporal.py`; post-merge CI passed on both platforms. Item 9B defines metadata-only applicability, baseline, complete standard-metric inventory, statistical-method, sample/robustness, frozen multiple-testing binding, and scientific evidence report contracts. Evidence plans and reports use existing JCS/SHA-256 identities, map the authoritative V0–V9 gates including V5 search adjustment, preserve unfavorable outcomes, and narrowly distinguish pending from completed evidence. They do not establish execution or experiment-decision authority. Item 9C is `NOT STARTED`; backtest inputs/outputs, transaction-cost protocols, and simulation remain separately gated. Item 10 is `NOT STARTED`. Do not implement formulas, strategies, a backtest engine, optimizer, or market simulator.
