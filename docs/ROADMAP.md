...
**Status: IN PROGRESS — ITEM 8A (2026-09-06).** Batches 1–2
...
8. **Experiment controls — IN PROGRESS / ITEM 8A:** Item 8A implements only governed `DRAFT → REGISTERED → FROZEN` transitions, a result-free preregistration gate, exact registered-revision freeze binding, caller-supplied transition timestamps, and immutable canonical freeze evidence. Multiple-testing runtime attempt counters, execution and later transitions, release-event integration, result/failure retention, and deterministic `rerun EXP-<uuidv7>` resolution remain separately gated. Do not implement a backtesting engine or strategy.
