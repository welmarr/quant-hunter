...
ITEM 10A IMPLEMENTED / INDEPENDENT REVIEW PENDING; ITEM 10B NOT STARTED
(2026-09-11).** Batches 1–2
...
coverage. Item 10A now implements the software release core and synthetic
security contracts described below, pending independent review. Item 10B and
...
10. **Sealed OOS boundary — 10A IMPLEMENTED / INDEPENDENT REVIEW PENDING; 10B NOT STARTED:** Item 10A adds only the software release core and synthetic security contracts. It verifies exact current Item 8 FROZEN authority for authorization, binds the complete canonical dataset identity set and exact sealed partition, verifies a synthetic immutable released artifact, and writes RFC 8785/SHA-256 events to an append-only CAS and hash-chained exposure ledger. `SealedReleaseService.authorize_release` is the sole supported public release writer and `record_accidental_exposure` is the sole supported public incident writer; the ledger offers public structural read/verification operations but no raw public append authority. Exposure is global to each dataset and exact half-open interval across experiments; any overlapping prior release or incident blocks authorization, while later incidents remain appendable as permanent evidence. Historical verification follows the exact sole FROZEN revision after Item 8 advances. A released experiment retains the exact event digest, rejects all later search attempts, and may continue only with a fixed zero-new-search evaluation. Synthetic authorization never reads, lists, stats, traverses, or hashes a sealed source. Item 10A cannot create or accept `HOST_ENFORCED` evidence. Item 10B remains separately gated to approved host changes, real custodian/research identities, encrypted storage, allow-only audited ACLs outside repository/cache/index/sync roots, backup/sync/index controls, and effective two-identity denial evidence. Item 10 is not complete until Item 10B passes its separate gate and review.
