...
10. **Sealed OOS boundary — 10A IMPLEMENTED / INDEPENDENT REVIEW PENDING; 10B NOT STARTED:** Item 10A adds only the software release core and synthetic security contracts. It verifies exact Item 8 FROZEN authority, binds the complete canonical dataset identity set and exact sealed partition, verifies a synthetic immutable released artifact, and writes RFC 8785/SHA-256 events to an append-only CAS and hash-chained exposure ledger. Release and accidental exposure are one-way `EXPOSED`; a released experiment retains the exact event digest when it enters `RUNNING`, rejects all later search attempts, and may continue only with a fixed zero-new-search evaluation. Synthetic authorization never reads, lists, stats, traverses, or hashes a sealed source. Item 10A cannot create or accept `HOST_ENFORCED` evidence. Item 10B remains separately gated to approved host changes, real custodian/research identities, encrypted storage, allow-only audited ACLs outside repository/cache/index/sync roots, backup/sync/index controls, and effective two-identity denial evidence. Item 10 is not complete until Item 10B passes its separate gate and review.
