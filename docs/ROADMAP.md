...
**Status: IN PROGRESS — ITEM 9A IN PROGRESS / REVIEW (2026-09-07).** Batches 1–2
...
9. **Validation and simulation interfaces only — IN PROGRESS / ITEM 9A REVIEW:** Item 9A defines immutable, configuration-driven metadata contracts for exact UTC half-open intervals, chronological training/development, validation, and sealed-OOS partitions, explicitly sequenced chronological-holdout, rolling, expanding, and purged time-aware folds, and exact purge/embargo evidence or reasoned non-applicability. Canonical JCS evidence and SHA-256 identity are deterministic. The package does not split tables, dereference sealed references, execute research, or establish that an experiment used a plan. Item 9B and Item 9C are not started; multiple-testing interfaces, baselines, metrics, statistical analyses, decision reports, backtest inputs/outputs, transaction-cost protocols, and simulation remain separately gated. Do not implement strategies, a backtest engine, optimizer, or market simulator.
