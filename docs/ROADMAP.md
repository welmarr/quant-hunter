...
**Status: IN PROGRESS — ITEM 8B REVIEW (2026-09-06).** Batches 1–2
...
`79730f9ed54d6fcf9c8b33ad70af6181941c0b5e`. Item 8 remains `IN PROGRESS`
through the explicitly authorized 8B running-state and runtime-attempt controls;
items 9–13 remain unauthorized and unstarted. Execute
...
8. **Experiment controls — IN PROGRESS / ITEM 8B REVIEW:** Item 8A is complete after independent review passed commit `79730f9ed54d6fcf9c8b33ad70af6181941c0b5e`; it implements governed `DRAFT → REGISTERED → FROZEN`, result-free preregistration, exact registered-revision freeze binding, caller-supplied full-precision UTC transition timestamps, and immutable canonical freeze evidence. Item 8B adds only independently verified `FROZEN → RUNNING` and append-only, CAS-protected evidence for every actual ordinary, AI-generated, failed, and retried search attempt. Counters are derived from retained attempt evidence and cannot exceed the frozen multiple-testing budget. Item 8B is pending independent review. Evaluation, decisions, final results, release-event integration, failure-artifact retention, and deterministic `rerun EXP-<uuidv7>` resolution remain separately gated. Do not implement a backtesting engine or strategy.
