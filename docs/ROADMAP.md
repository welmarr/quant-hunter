...
10. **Sealed OOS boundary — 10A COMPLETE / INDEPENDENT REVIEW PASSED / MERGED / POST-MERGE CI GREEN; 10B AUTHORIZED / EVIDENCE GATED:** Item 10A adds only the software release core and synthetic security contracts. It verifies exact current Item 8 FROZEN authority for authorization, binds the complete canonical dataset identity set and exact sealed partition, verifies a synthetic immutable released artifact, and writes RFC 8785/SHA-256 events to an append-only CAS and hash-chained exposure ledger. `SealedReleaseService.authorize_release` is the sole supported public release writer and `record_accidental_exposure` is the sole supported public incident writer; the ledger offers public structural read/verification operations but no raw public append authority. Exposure is global to each dataset and exact half-open interval across experiments; any overlapping prior release or incident blocks authorization, while later incidents remain appendable as permanent evidence. Historical verification follows the exact sole FROZEN revision after Item 8 advances. A released experiment retains the exact event digest, rejects all later search attempts, and may continue only with a fixed zero-new-search evaluation. Item 10A passed independent review at head `0892bfdb9231053e8896867facb8fc3de47ebf8e`, merged on main at `20851f262041cda1fe26844032f298b2a1531ffd`, and passed post-merge Quality #36 on Ubuntu and Windows. Item 10B remains separately gated to an already protected fixed NTFS volume, approved host changes, real custodian/research identities, inheritance-disabled allow-list ACLs, filesystem auditing, index/sync/backup evidence, controlled release, and effective two-identity denial evidence. It may not alter BitLocker in this batch. Item 10 is not complete until Item 10B captures its host evidence and passes independent review.
