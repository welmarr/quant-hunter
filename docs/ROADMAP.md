...
**Status: IN PROGRESS — ITEM 9B IN PROGRESS / REVIEW (2026-09-07).** Batches 1–2
...
Item 9B scientific-evidence contracts; Item 9C and items 10–13 remain unstarted. Execute
...
9. **Validation and simulation interfaces only — IN PROGRESS / ITEM 9B REVIEW:** Item 9A is complete after independent review and defines immutable, configuration-driven metadata contracts for exact UTC half-open intervals, chronological partitions, explicitly sequenced time-aware folds, and fold-local purge/embargo evidence. Final hosted evidence was 488 passing tests on Ubuntu and Windows, 91.46% Ubuntu combined statement/branch coverage, and 99.34% coverage for `temporal.py`; post-merge CI passed on both platforms. Item 9B defines metadata-only applicability, baseline, complete standard-metric inventory, statistical-method, sample/robustness, frozen multiple-testing binding, and scientific evidence report contracts. Evidence plans and reports use existing JCS/SHA-256 identities, preserve unfavorable and pending outcomes, and do not establish execution or experiment-decision authority. Item 9C is not started; backtest inputs/outputs, transaction-cost protocols, and simulation remain separately gated. Do not implement formulas, strategies, a backtest engine, optimizer, or market simulator.
