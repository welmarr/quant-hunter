...
| RISK-019 | Canonicalization, floating-point representation, Parquet versions, or row order causes misleading digest drift or false equivalence. | JCS conformance vectors; precision-sensitive strings; physical and semantic manifest hashes; explicit ordering; pinned environment; fail on mismatch rather than normalizing silently. | OPEN |
