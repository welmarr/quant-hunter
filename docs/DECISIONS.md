...
### DEC-0034 — Make GitHub Issues the Operational Tracker for Deferred Material Work

- **Date:** 2026-09-11
- **Status:** ACCEPTED
- **Scope:** project continuity / operational governance / reproducibility
- **Context:** Durable repository documents preserve scientific meaning, decisions, invariants, risk evidence, roadmap gates, and the current resume point, but actionable material work can still be lost between sessions when it is outside the current authorized batch. A remote work tracker is needed without allowing deferred labels to bypass current correctness gates.
- **Decision:** GitHub Issues are the operational tracker for material deferred work. Repository documents remain authoritative for scientific and governance meaning. A material problem requires an Issue when it is evidenced or credible, unresolved in the current reviewed batch, needs future work, and would be harmful to lose. Each Issue records exact evidence, impact, confirmed facts and unknowns, why it is deferred, an exact Stage/Item/gate target, investigation and proposed solution, acceptance criteria, regression evidence, durable authority relationships, technical references, and closure evidence. Current-batch correctness failures cannot be converted into a pass by Issue creation. Agents inspect open material Issues during resume/preflight, search for duplicates, and link Issues back to applicable risk, decision, regression, project-status, and roadmap records.
- **Alternatives considered:** Repository documents alone do not provide an actionable remote queue. Issues alone would make editable operational records the scientific authority. Informal “later” notes have no owner, target, or closure proof. Automatically issuing every minor cleanup item would obscure material work.
- **Scientific/statistical consequences:** No scientific invariant or validation gate changes. Defects affecting current scientific correctness still block the batch. Deferred scientific or reproducibility debt becomes harder to lose and retains explicit acceptance evidence.
- **Reproducibility and security consequences:** An Issue may close only after its stated criteria and durable evidence exist; belief, a non-recurrence, one successful rerun, a timeout increase, a workaround, or related code change is insufficient. Partial resolutions remain open. Closure reconciles affected repository authorities. This governance adds no dependency, service purchase, data, infrastructure, or usage commitment; incremental direct cost is USD 0.
- **References:** `AGENTS.md`, `docs/ISSUE_GOVERNANCE.md`, `.github/ISSUE_TEMPLATE/material-deferred-work.yml`, `docs/DEFERRED_ISSUE_DRAFTS.md`, `PROJECT_STATUS.md`, `REGRESSION_GUARD.md`, `ROADMAP.md`, and `RISK_REGISTER.md`.
- **Supersedes / superseded by:** Extends DEC-0029 continuity governance with an operational deferred-work tracker. It does not replace or weaken any durable repository authority.
- **Owner and approver:** Project owner through the explicitly authorized overnight deferred-issue governance and Item 10B batch dated 2026-09-11.

### DEC-0035 — Establish the Windows Host-Enforced Sealed OOS Boundary

- **Date:** 2026-09-11
- **Status:** ACCEPTED — IMPLEMENTED TOOLING / HOST EVIDENCE BLOCKED
- **Scope:** Stage 1B Item 10B / local Windows sealed-OOS isolation
- **Context:** Item 10A supplies reviewed software release authority and synthetic exposure evidence, but it deliberately cannot prove operating-system isolation. Item 10B needs a separate host authority without allowing a raw mapping, a generic release request, or a software-only test to claim real enforcement.
- **Decision:** Preserve `SealedReleaseService.authorize_release` as the sole `SYNTHETIC_TEST` release writer and Item 8 as the sole experiment lifecycle/attempt authority. Add a Windows-only verifier that converts a complete live harness report into exclusively published, schema-valid RFC 8785 evidence whose SHA-256 identity excludes only its own digest. A `HOST_ENFORCED` event requires that exact typed evidence, the effective `qh-oos-custodian` identity, a matching release-object root, exact FROZEN authority and sealed binding, an immutable released artifact, and the existing private ledger append, CAS, chain, overlap, and one-way `EXPOSED` controls. The host design requires an already protected fixed NTFS volume, separate non-administrator custodian and research identities, inheritance-disabled allow-list DACLs, focused File System SACL/audit evidence, index exclusion, no consumer-sync overlap, backup assessment, effective research denial, custodian publication, and research-readable immutable release evidence. Authentication material is generated and retained only in memory for the effective-identity probes; the test logons are disabled afterward when no approved durable secret manager exists.
- **Alternatives considered:** Letting Item 10A accept a mode string would permit a software-only host claim. A raw evidence mapping would bypass the live verifier. Same-account folders, hidden paths, archive passwords, or an unencrypted directory do not establish the boundary. Enabling or altering BitLocker in an unattended batch exceeds the authorization and recovery-key risk boundary.
- **Scientific/statistical consequences:** Successful host evidence would enforce the sealed-data access gate around the exact frozen experiment while preserving the Item 8 lifecycle and global exposure rules. Tooling or unit tests alone do not establish confidentiality, permit evaluation, or complete Item 10.
- **Reproducibility and security consequences:** The 2026-09-11 read-only preflight confirmed Windows and two fixed NTFS volumes but ran without elevation; BitLocker, File System audit-policy, and backup state could not be proven. Therefore no users, paths, DACLs, SACLs, audit settings, indexing attributes, or host evidence were created. BitLocker was not changed. Status remains `IMPLEMENTED TOOLING / HOST EVIDENCE BLOCKED`. The accepted threat model addresses accidental, workflow, and research-identity access. Administrators, SYSTEM, malicious kernel code, and a fully compromised administrator remain outside it; unknown backup agents and future credential activation remain residual risks. Only synthetic fixtures are allowed. No dependency, service, data, or infrastructure purchase is added; incremental direct cost is USD 0.
- **References:** DEC-0006; DEC-0031–DEC-0034; RISK-017; REG-F01–REG-F03; `ARCHITECTURE.md`; `DATA_ARCHITECTURE.md`; `windows-host-boundary-evidence.schema.json`; `sealed-release-event.schema.json`; `isolation/windows_host.py`; `scripts/windows/item10b_*`; `tests/test_windows_host.py`; `tests/test_windows_host_scripts.py`; and `docs/host-evidence/item10b-preflight-2026-09-11.md`.
- **Supersedes / superseded by:** Implements the bounded Item 10B architecture while retaining its live evidence gate. It does not weaken or supersede Item 10A, close RISK-017, complete Item 10, authorize real data, or start Item 11.
- **Owner and approver:** Project owner through the explicitly authorized overnight deferred-issue governance and Item 10B batch dated 2026-09-11.

### DEC-0036 — Prevent Raw Host Reports from Acquiring HOST_ENFORCED Authority

- **Date:** 2026-09-11
- **Status:** ACCEPTED
- **Scope:** security / scientific authority / reproducibility
- **Context:** Independent review found that the first Item 10B implementation exposed `WindowsHostBoundaryVerifier.finalize_live_report`. That method could schema-validate and canonicalize caller-authored JSON into `VerifiedWindowsHostBoundaryEvidence` without independently establishing the claimed host facts. Schema validity proves record structure, not a Windows host observation.
- **Decision:** Caller assertions, raw mappings, JSON documents, and arbitrary report files cannot become `HOST_ENFORCED` authority. `capture_live_evidence` is the sole supported live authority-creation operation: on Windows, under an elevated administrator, and only after explicit caller setup authorization, it invokes the governed setup workflow itself, consumes only that execution's immediate result, binds the exact successful preflight observations to the effective identity/ACL/audit/release probes, and exclusively publishes canonical evidence. The standalone CLI delegates to this operation and accepts no report path. The exact preflight chain covers the candidate-root fingerprint, fixed NTFS and BitLocker observations, repository/worktree and profile/cache/temp/sync exclusions, pre-existing identity/path checks, original File System audit-policy state, and backup classification. Retained canonical evidence may still be loaded and reverified through `load`; loading is not a raw-report finalization route.
- **Alternatives considered:** Renaming the raw finalizer or retaining a CLI `--live-report` argument would preserve the bypass. Trusting successful booleans from a second script would not bind the preflight used by setup. Removing retained-evidence loading would prevent later audit without correcting the creation authority.
- **Scientific/statistical consequences:** A `HOST_ENFORCED` claim can originate only from one governed executed workflow. Failed or substituted preflight observations and failed research denial, custodian access, released-artifact immutability, DACL, SACL/audit, sync, encryption, or indexing checks produce no authority. Item 8 and the reviewed Item 10A release and exposure invariants remain unchanged.
- **Reproducibility and security consequences:** Python-private operations limit accidental and workflow misuse; administrators, SYSTEM, or arbitrary code already running with administrative authority remain outside the Stage 1 threat model. Item 10B live evidence remains `BLOCKED`: no successful live evidence was created, no real sealed data was used, and no BitLocker setting was changed. RISK-017 remains `OPEN`. No dependency, paid service, data, or infrastructure is added; incremental direct cost is USD 0.
...
### DEC-0037 — Make Repository and GitHub Evidence the Reviewer-Continuity Authority

- **Date:** 2026-09-18
- **Status:** ACCEPTED
- **Scope:** methodology / process / project continuity
- **Context:** Nova performs independent review in a separate product and cannot rely on prior chat history. Product-generated memory is not guaranteed to be shared, durable, regionally available, or independently auditable. Continuity needs evidence-first reconstruction without forcing each review to mutate the reviewed head or follow an oversized response ceremony.
- **Decision:** Continuity across Nova review sessions is enforced through versioned repository authorities and live GitHub evidence. `PROJECT_STATUS.md` is the compact durable resume snapshot. `SESSION_LOG.md` is optional and may index only major `CLOSED` or `BLOCKED` checkpoints when naturally included in a later governed change; it is never required to validate a PR and the reviewed PR must not change solely to record its own review. No chat product's generated memory, including a machine-local assistant-memory layer, is project authority. Substantive Nova reviews use the compact core `Evidence → Reconstructed State → Verdict + Findings → Next Gate`; missing evidence blocks a verdict. PR packages, model/reasoning recommendations, Codex prompts, and checkpoint blocks are conditional. Nova recommends an available model/configuration and reasoning effort appropriate to task complexity; product model names are not durable governance authority. The owner is the sole PR creator, sole human merge authorizer, and sole merge executor. Nova's `#CASEFAILED` prohibits governed merge readiness; `#CASEPASSED` satisfies only the technical gate and requires a separate owner decision. One coherent goal or review unit uses one branch and one PR, including in-scope fixes, tests, and directly related documentation; failed review remains on that branch and PR, while a genuinely distinct objective starts another.
- **Alternatives considered:** Conversation continuity and product memory are convenient but not durable authority. A historical or transient `PROJECT_STATUS.md` would duplicate Git and self-invalidate. A mandatory per-review session-log write creates an endless reviewed-SHA loop. Allowing Nova or Codex to create or merge PRs would replace the owner's explicit gate.
...
- **References:** `AGENTS.md`; `docs/WORKING_PROTOCOL.md`; `docs/PROJECT_STATUS.md`; optional `docs/SESSION_LOG.md`; `docs/PENDING_ISSUES_DRAFT.md`; DEC-0029; DEC-0030; DEC-0034.
...
### DEC-0038 — Require the Synthetic Tracer as the Stage 1 Exit Integration Gate

- **Date:** 2026-09-18
- **Status:** ACCEPTED — EXECUTION DEFERRED
- **Scope:** methodology / integration validation / stage gating
- **Context:** External review observed that the individual experiment-lifecycle and V0–V9 contracts have extensive synthetic tests, but no experiment has traversed the full `DRAFT → REGISTERED → FROZEN → RUNNING → EVALUATED → DECIDED` path as one retained end-to-end workflow. Component completion alone cannot prove the integrated laboratory.
- **Decision:** Finish the ordered Stage 1B Items 10B–13 first, then run at least one synthetic end-to-end tracer as the Stage 1 Exit Integration Gate. The tracer must use a permanent experiment ID, governed authority bindings, attempt accounting, applicable V0–V9 disposition and evidence, retained artifacts, deterministic reconstruction, and independent review. Stage 1B Item 13 closeout does not waive this gate: Stage 1 becomes complete only after the tracer passes, and Stage 2 remains locked until then. The tracer creates no strategy, real-data, live-trading, or production authority.
- **Alternatives considered:** Running the tracer immediately would interrupt the current Item 10B → Item 11 → Item 12 → Item 13 sequence. Treating it as an arbitrary post-Stage-1B/pre-Stage-2 task would obscure that it is the integration proof required to complete Stage 1. Omitting it would let separate component tests substitute for proof of the integrated research workflow.
...
- **References:** `docs/ROADMAP.md` Stage 1 Exit Integration Gate; `docs/EXPERIMENT_LEDGER.md`; `docs/VALIDATION_STANDARD.md`; REG-008–REG-018; tracer draft in `docs/PENDING_ISSUES_DRAFT.md`.
- **Supersedes / superseded by:** Adds the Stage 1 exit integration proof without reopening the reviewed Item 8 or Item 9 contracts and without changing the current Stage 1B item order.
...
- **Date:** 2026-09-18
- **Status:** ACCEPTED — REVIEW DEFERRED TO TRIGGER
